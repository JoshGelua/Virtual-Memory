...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		// If the page table is dirty
		if (victim->frame & PG_DIRTY){
			// set victim page to swap
		...
		victim->frame &= ~PG_DIRTY; // Clean since it is saved
	...
	coremap[frame].pte = p;
	coremap[frame].referenced = 0;
	coremap[frame].next = NULL;
	coremap[frame].prev = NULL;
...
char *find_physpage(addr_t vaddr, char type) {
	...
	unsigned page_index = PGTBL_INDEX(vaddr);
	pgtbl_entry_t *page_table = (pgtbl_entry_t *)(pgdir[idx].pde);
	p = page_table + page_index;

	// Check if p is valid or not, on swap or not, and handle appropriately
	if (p->frame & PG_VALID){
		if (p->frame & PG_ONSWAP){
			// If p is valid and on swap
			// This is not possible
			perror("find_physpage");
			exit(1);
		}
		else{
			// If p is valid and not on swap
			hit_count++;
			ref_count++;
		}
	}
		...
		ref_count++;
		if (p->frame & PG_ONSWAP){
			// If p is not valid and on swap
			// allocate a frame and swap
			...
			// If p is not valid and not on swap
	...
	// If the page will be modified or stored, mark dirty
	if (type == 'M' || type == 'S'){
		p->frame |= PG_DIRTY;
	}
