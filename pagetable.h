...
struct frame {
	...
	int referenced; 	// ADDED referenced to mark that the algo has performed ref
										// on this frame (has referenced it)
	struct frame *next; // ADDED next and prev to allow
	struct frame *prev;	// Doubly-linked lists for LRU
};
